...
MySQLStore.prototype.failsForField = function (field, value, cb) {
  var where = ' AND `' + field + '` = ' + this.connection.escape(value)
  var sql = 'SELECT'
          + ' COUNT(1) AS fails,'
          + ' UNIX_TIMESTAMP('
          ...
          + 'FROM `loginAttempts` '
          + 'WHERE '
          + ' `attempted` >= (SELECT MAX(`attempted`) FROM `loginAttempts` WHERE `success` = true' + where + ') '
          + 'AND '
          + ' `success` = false'
          + where
...
MySQLStore.prototype.knownRemoteAddr = function (remoteAddr, account, cb) {
  ...
  var sql = 'SELECT EXISTS('
          ...
          + ' WHERE'
          + '   `remoteAddr` = ' + conn.escape(remoteAddr)
